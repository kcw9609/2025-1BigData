...
"""
import json
with open('./output/빅데이터_naver_news.json', 'r', encoding='utf-8') as f:
...
# macOS에서는 'AppleGothic' 폰트를 사용하는 것이 일반적
rc('font', family='AppleGothic')

# 마이너스 기호 깨짐 방지
plt.rcParams['axes.unicode_minus'] = False

# 상위 300개 단어 가져오기 (예시로 ko.vocab()이 있다고 가정)
data = ko.vocab().most_common(300)
print(len(data))

# 리스트 튜플을 딕셔너리로 바꾸는 함수
...
# 워드클라우드 그리기 (macOS용 폰트 경로 지정)
wordcloud = WordCloud(
    font_path='/System/Library/Fonts/AppleGothic.ttf',  # macOS 기본 한글 폰트
...
from matplotlib import font_manager, rc
font_name = font_manager.FontProperties(fname='c:/Windows/Fonts/apple.ttf').get_name()
