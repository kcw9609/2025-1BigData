...
# coding: utf-8
# encoding = 'cp949'
# plt.rc('font', family ='AppleGothic') 로 수정해야함

# In[1]:
# unit14 numpy 내용 - numpy 제거하고 실행해봄.
# age 파일 다루기 - 행정구역별 연령별 인구수
import csv   
f =open('age.csv', encoding='cp949')  # 파일이 존재한다는 가정   # 행정구역, 총인구수, 연령구간인구수, 0세, 1세, 2세, ...
data = csv.reader(f)
next(data)  # 헤더 제거
for row in data :
    print(row)
...
# In[29]: 동끼리 비교해서 비율의 차가 작은 곳을 구함 하지만 차트 결과는 의미있지 않음 -> In34로 이동
import numpy as np
import csv
f =open('age.csv', encoding='cp949') 
data = csv.reader(f)
...
for row in data :
    if name in row[0]: #입력 받은 지역의 이름이 포함된 행 찾기
        ...
        hometotal=int(row[2])
for k in range(len(home)):
    home[k]=(home[k]/hometotal) # ➊나잇대별로 비율을 구함
...
# In[34]: 차이를 제곱하여 격차를 정확하게 파악할 수 있다
import numpy as np
import csv
#1. 데이터를 읽어온다.
f =open('age.csv', encoding='cp949') 
...
for i in range(10):
    plt.plot(result_list[i+1][1], label = result)
plt.legend()
plt.show()
